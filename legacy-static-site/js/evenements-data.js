...
export const baselineEvenements = [
  {
    ...
    badgeColor: "success",
    description: "Rencontrez plus de 45 recruteurs (télécoms, banques, pétrole, logistique, numérique) et déposez directement vos CV imprimés ou via QR code Sala.",
  ...
  },
  {
    ...
    badgeColor: "primary",
    description: "Atelier pratique interactif pour maîtriser la rédaction de CV aux normes internationales et réussir ses entretiens face aux DRH.",
  ...
  },
  {
    ...
    price: "100% En Ligne",
    badgeColor: "info",
    description: "Découvrez les compétences technologiques les plus recherchées par les entreprises régionales et internationales en télétravail.",
  ...
  },
  {
    ...
    price: "Accès libre",
    badgeColor: "warning",
    description: "Journée portes ouvertes et recrutement pour les profils techniques, logistiques, maritimes et supply-chain.",
    highlights: ["Entreprises portuaires", "Dépôt instantané de candidatures"],
    registrationRequired: true
  }
];

// Chargeur avec fallback résilient
export async function loadEvenements() {
  try {
    const q = query(collection(db, "evenements"), orderBy("date", "asc"));
    const snapshot = await getDocs(q);
    if (!snapshot.empty) {
      return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    }
  } catch (error) {
    console.warn("Firestore collection 'evenements' non disponible, utilisation des événements de base Sala :", error);
  }
  return baselineEvenements;
}
